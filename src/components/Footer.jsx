      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-10 mb-14">
          ...
          <div className="col-span-2">
            <a href="#" className="flex items-center gap-2.5 mb-5 w-fit">
              <div className="w-9 h-9 rounded-xl bg-linear-to-br from-purple-500 to-pink-500 flex items-center justify-center">
                <svg viewBox="0 0 24 24" fill="none" className="w-5 h-5 text-white">
                  <path d="M9 3l-6 9h5l-2 9 14-12h-6l3-6z" fill="currentColor"/>
                </svg>
              </div>
            ...
            <div className="flex gap-3">
              {['twitter', 'linkedin', 'github', 'youtube'].map((s) => (
                <a key={s} href="#" className="w-9 h-9 rounded-lg glass flex items-center justify-center text-slate-400 hover:text-purple-400 hover:border-purple-500/30 transition-all duration-300">
          ...
          {Object.entries(LINKS).map(([cat, links]) => (
            <div key={cat}>
              <p className="text-white text-sm font-semibold mb-4">{cat}</p>
              <ul className="space-y-2.5">
                {links.map(link => (
                  <li key={link}>
                    <a href="#" className="text-slate-500 text-sm hover:text-slate-300 transition-colors duration-200">
                      {link}
                    </a>
                  </li>
                ))}
